...
namespace Game {
	...
	void HLSLShaderLoader::AddShaderIncludePath(const char* path) {
		gFileLoader.AddFileSearchPath(path);
	}
	
	ComPtr<ID3DBlob> HLSLShaderLoader::CompileFile(const char* filename,const char* entry,
	...
	ComPtr<ID3DBlob> HLSLShaderLoader::CompileFormMemory(const char* code,size_t codeSize,const char* entry,
		SHADER_TYPE type,const D3D_SHADER_MACRO* macros,UINT compileFlag) {
		ComPtr<ID3DBlob> target, errorCode;

		HRESULT hr = D3DCompile(code, codeSize, nullptr, macros,&sInclude,entry,target_list[type],
			compileFlag,0,&target,&errorCode);

		if (FAILED(hr)) {
			Log("shader loader fail to compile shader code at entry %s\n", entry);
			Log("\nreason : %s \n",errorCode->GetBufferPointer());
			return nullptr;
		}
