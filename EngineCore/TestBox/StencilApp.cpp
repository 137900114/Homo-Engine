...
private:
	...
	std::unique_ptr<SobelFilter> mSobel;
...
bool StencilApp::Initialize()
{
	...
	mSobel = std::make_unique<SobelFilter>(mClientWidth,mClientHeight,md3dDevice.Get(),mBackBufferFormat);
	mSobel->CreateRootSig();
	mSobel->CreatePSO(L"Shaders\\Sobel.hlsl","SobelC");
...
void StencilApp::OnResize()
{
	...
	if (mSobel != nullptr)
		mSobel->Resize(mClientWidth, mClientHeight);
}
...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	mSobel->Excute(mCommandList.Get(), CurrentBackBuffer());
...
void StencilApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	a = MathHelper::Clamp(a, 0.f, 2.f);
	// Don't let user move below ground plane.
	mSkullTranslation.y = MathHelper::Max(mSkullTranslation.y, 0.0f);

	// Update the new world matrix.
	XMMATRIX skullRotate = XMMatrixRotationY(a *MathHelper::Pi);
...
void StencilApp::BuildDescriptorHeaps()
{
	...
	CD3DX12_GPU_DESCRIPTOR_HANDLE hGHandle(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	...
	hGHandle.Offset(1, mCbvSrvDescriptorSize);
	...
	mSobel->CreateDescriptor(hDescriptor, hGHandle, mCbvSrvDescriptorSize);
}
